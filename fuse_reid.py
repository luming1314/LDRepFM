...
def parse_opt() -> Namespace:
    ...
    parser.add_argument('--weights', type=str, default='cache/a1/187.pth', help='pretrained weights path')
    parser.add_argument('--deploy_weight', type=str, default='cache/a2/187.pth', help='pretrained weights path')
    ...
    parser.add_argument('--card', metavar='MODE', default='multi', choices=['multi', 'single'],
                        help='train is multi or single')
...
if __name__ == '__main__':
    ...
    ir_paths = []
    testpath_ir = "./data/test/RegDB/Thermal"
    for root, dirs, files in sorted(os.walk(testpath_ir, topdown=False)):
        for name in sorted(files):
            bb = os.path.join(root, name)
            bb = Path(bb)
            ir_paths.append(bb)

    vi_paths = []
    testpath_vi = "./data/test/RegDB/Visible"
    for root, dirs, files in sorted(os.walk(testpath_vi, topdown=False)):
        for name in sorted(files):
            bb = os.path.join(root, name)
            bb = Path(bb)
            vi_paths.append(bb)
