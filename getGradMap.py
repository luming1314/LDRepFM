...
def parse_opt() -> Namespace:
    ...
    # universal opt
    parser.add_argument('--src', type=str, help='fusion data root path  DataSets include:[TNO/RoadScene/MSRS/M3FD]', default='data/train/M3FD' )
...
if __name__ == '__main__':
    ...
    save_path = './data/train/M3FD/SLab'
    # images
    root = Path(config.src)
    ir_paths = [x for x in sorted((root / 'IR').glob('*')) if img_filter]
    vi_paths = [x for x in sorted((root / 'VIS').glob('*')) if img_filter]
    ...
    f.getGradMap(ir_paths, vi_paths, Path(save_path), config.color)
